...
function navigateToStreamingPage(contentItem) {
    console.log('navigateToStreamingPage:', contentItem);
    // Build the URL with query parameters
    const url = new URL('streaming.html', window.location.origin);
    
    // If origin is "Local", use local path; otherwise use Google Drive API
    if (contentItem.origin === 'Local') {
        console.log('Origin is Local, using videoUrl:', contentItem.localPath);
        url.searchParams.set('videoUrl', contentItem.localPath);
    } else {
        console.log('Origin is Drive, using folderId:', contentItem.folderid);
